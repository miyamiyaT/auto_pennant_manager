...
RSpec.describe 'Api::V1::Batter', type: :request do
  describe 'GET' do
    ...
    it "returns batter detail" do
      ...
      get "/api/v1/batters/#{player.id}/register"

      expect(response).to have_http_status(:ok)
      json = JSON.parse(response.body)

      expect(json).to include("player_data")
      expect(json['player_data']).to include('player', 'player_season', 'batter_season', 'batter_ability')
    end
  end
    ...
    it '野手登録が成功し 201 を返す' do
      post '/api/v1/batters', params: params
