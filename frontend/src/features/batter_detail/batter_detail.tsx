...
import PlayerResultTableContent from './player_result_table_content';
...
const BatterTable = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/v1/batters/${id}`)
      ...
      <Box mt={5}>
        {playerDetails.player.map(player => (
          <Box key={player.id} mb={4}>
            ...
            <PlayerResultTableContent player_seasons={player.player_seasons} />
            <br></br>
            <br></br>
            <Typography variant="h5" gutterBottom>
              野手能力: {player.name}
            </Typography>
            <PlayerAbilityTableContent player_seasons={player.player_seasons} />
          </Box>
        ))}
      </Box>
    </Container>
